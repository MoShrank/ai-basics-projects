...
# - take_out_bread will move the bread from toaster to plate. 
#        This is only possible if the toaster is switched off.
...
def breadth_first_search(start_state):
    ...
    while len(to_do) > 0:
        ...
        for action in actions:
            ...
            to_do.append(([*seq, action], new_state))
...
def heuristic_search(start_state):
    ...
    while len(to_do) > 0:
        ...
        for action in actions:
            # check if action is already in sequence
            #if action not in seq:    
            new_state = state_transition(current_state, action)
            to_do.append(([*seq, action], new_state))
            # sort to_do actions by cost
            to_do = sorted(to_do, key=lambda item: item[1]['time']) 

    return None        

def plan(start_state):
    return heuristic_search(start_state)
